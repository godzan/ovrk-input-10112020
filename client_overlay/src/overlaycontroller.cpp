...
void OverlayController::Shutdown() {
	...
	if (m_pUpdateVrControllerTimer) {
		disconnect(m_pUpdateVrControllerTimer.get(), SIGNAL(timeout()), this, SLOT(OnTimeoutUpdateVrController()));
		m_pUpdateVrControllerTimer->stop();
		m_pUpdateVrControllerTimer.reset();
	}
	if (m_pRenderTimer) {
...
void OverlayController::SetWidget(QQuickItem* quickItem, const std::string& name, const std::string& key) {
	...
	m_pUpdateVrControllerTimer.reset(new QTimer());
	connect(m_pUpdateVrControllerTimer.get(), SIGNAL(timeout()), this, SLOT(OnTimeoutUpdateVrController()));
	m_pUpdateVrControllerTimer->setInterval(1);
	m_pUpdateVrControllerTimer->start();
...
void OverlayController::OnTimeoutUpdateVrController() {
	if (!vr::VRSystem())
		return;
	keyboardInputTabController.updateVrController();
}
