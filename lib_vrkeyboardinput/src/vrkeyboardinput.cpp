...
#if VRKEYBOARDINPUT_EASYLOGGING == 1
#include "logging.h";
...
namespace vrkeyboardinput {
	...
	void VRKeyboardInput::_ipcThreadFunc(VRKeyboardInput * _this) {
	...
	VRKeyboardInput::VRKeyboardInput(const std::string& serverQueue, const std::string& clientQueue) : _ipcServerQueueName(serverQueue), _ipcClientQueueName(clientQueue) {}

	VRKeyboardInput::~VRKeyboardInput() {
		disconnect();
	}

	bool VRKeyboardInput::isConnected() const {
		return _ipcServerQueue != nullptr;
	}

	void VRKeyboardInput::connect() {
		if (!_ipcServerQueue) {
			...
			try {
				...
				throw vrkeyboardinput_connectionerror(ss.str());
			}
			...
			if (resp.status != ipc::ReplyStatus::Ok) {
				...
				if (resp.status == ipc::ReplyStatus::InvalidVersion) {
					...
					throw vrkeyboardinput_invalidversion(ss.str());
				}
	...
	void VRKeyboardInput::disconnect() {
		if (_ipcServerQueue) {
	...
	void VRKeyboardInput::ping(bool modal, bool enableReply) {
		if (_ipcServerQueue) {
			...
			if (modal) {
				...
				if (resp.status != ipc::ReplyStatus::Ok) {
					...
					throw vrkeyboardinput_exception(ss.str());
				}
			}
			else {
				if (enableReply) {
			...
			throw vrkeyboardinput_connectionerror("No active connection.");
		}
	}

	void VRKeyboardInput::openvrHMDAdded(uint32_t deviceId) {
		if (_ipcServerQueue) {
	...
	void VRKeyboardInput::openvrFollowDevice(uint32_t deviceId) {
		if (_ipcServerQueue) {
	...
	void VRKeyboardInput::openvrUpdatePose(uint32_t deviceId, vr::DriverPose_t pose) {
		if (_ipcServerQueue) {
	...
	void VRKeyboardInput::openvrUpdateState(uint32_t deviceId, vr::VRControllerState_t new_state) {
		if (_ipcServerQueue) {
	...
	void VRKeyboardInput::openvrButtonEvent(ButtonEventType eventType, uint32_t deviceId, vr::EVRButtonId buttonId, double timeOffset) {
		if (_ipcServerQueue) {
	...
	void VRKeyboardInput::openvrAxisEvent(uint32_t deviceId, uint32_t axisId, const vr::VRControllerAxis_t & axisState) {
		if (_ipcServerQueue) {
	...
	void VRKeyboardInput::openvrEnableDriver(bool val) {
		if (_ipcServerQueue) {
