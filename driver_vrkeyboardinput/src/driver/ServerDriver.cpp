...
namespace vrkeyboardinput {
	namespace driver {
		...
		ServerDriver::~ServerDriver() {
			LOG(TRACE) << "CServerDriver::~CServerDriver_VRKeyboardInput()";
		}
		...
		void ServerDriver::openvr_enableDriver(bool val) {
			try {
				if (!initDriver && val) {
					...
					vr_locomotion1 = VirtualController("ovrki_000", true, test_pose, test_state);

					vr::VRServerDriverHost()->TrackedDeviceAdded("ovrki_000", vr::ETrackedDeviceClass::TrackedDeviceClass_Controller, &vr_locomotion1);
